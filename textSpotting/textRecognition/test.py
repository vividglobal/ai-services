...
from project_utils import CTCLabelConverter, AttnLabelConverter, Averager
...
def validation(model, criterion, evaluation_loader, converter, opt, log=None,):
    ...
    with torch.no_grad():
        if opt.benchmark_all_eval:  # evaluation with 10 benchmark evaluation datasets
            ...
            _, accuracy_by_best_model, _, _, _, _, _, _ = validation(
                model, criterion, evaluation_loader, converter, opt, log)
            log.write(eval_data_log)
...
if __name__ == '__main__':
    ...
    """ Data processing """
    parser.add_argument('--batch_max_length', type=int, default=90, help='maximum-label-length')
    parser.add_argument('--imgH', type=int, default=32, help='the height of the input image')
    parser.add_argument('--imgW', type=int, default=320, help='the width of the input image')
    parser.add_argument('--rgb', action='store_true', help='use rgb input')
    # parser.add_argument('--character', type=str, default=""" ĐịỌwÈỲơếềửcvrnỚọJhƠờỦLđẬồ.ễMÍẻẽ*ằE6ầỆẶ+Ó=ãỡàuớứK“ậòOỴỞừVÂXẼ&ẳƯỄạAấZ\'!ặCẢQỬIèlốỰéắTkBxPaẾổỏỒăjôYÉỜộỖ#>ÙỮ:Ấfì2eểù8(9Ẹ;ÌSẮủÊỨẪẩẵ57ỷÔỹHgmÀẠỢợ|3tỀF/sÃẦỤỠĨ0ỉâNRỪ”iDỳ1?úÁĂũ"ỊU~WẫzGưẰyóỵb,ỘỸ4qụdỔÚp)õẹêoệỗÝ%ảựỎáĩýởỂ`ỐẺíỈữẨ-""", help='character label')
